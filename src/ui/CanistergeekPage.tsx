...
import {CanisterSectionComponent} from "./CanisterSectionComponent";
...
const getContentComponent = (section: string): [React.ReactNode, React.ReactNode] => {
    switch (section) {
        case "summary":
            return [<SummarySectionComponent/>, <Space direction={"horizontal"}><SummaryForceCollectMetricsButtonComponent/><SummaryRefreshComponent/></Space>]
        default:
            return [<CanisterSectionComponent canisterId={section}/>, <Space direction={"horizontal"}><CanisterForceCollectMetricsButtonComponent canisterId={section}/><CanisterRefreshButtonComponent canisterId={section}/></Space>]
    }
}
...
export const CanistergeekPage = () => {
    const urlPathContext = useURLPathContext();
    const configurationContext = useConfigurationContext();
    const history = useHistory();
    const routeMatch = useRouteMatch<URLParams>(urlPathContext.basePath);
    ...
    if (!canisterIdInURLValid) {
        return <Redirect to={urlPathContext.pathToSection("summary")}/>
    }

    const [contentComponent, refreshComponent] = getContentComponent(canisterIdInURL)
    return <>
        <PageHeader title={`${PRODUCT_NAME}: Dashboard`} extra={refreshComponent}/>
        <PageContent className={"canistergeekPage"}>
