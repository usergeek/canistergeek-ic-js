...
import {Col, Descriptions, PageHeader, Result, Row, Spin, Typography} from "antd";
...
export const CanisterSectionComponentCanisterSource = (props: Props & PropsWithMetricsSource) => {
    ...
    const canisterLoaded = dataContext.status[canisterId]?.loaded
    ...
    const dataHourlyNotEmpty = !_.isNil(dataContext.dataHourly[canisterId])
    const dataDailyNotEmpty = !_.isNil(dataContext.dataDaily[canisterId])
...
export const CanisterSectionComponentBlackholeSource = (props: Props & PropsWithMetricsSource) => {
    ...
    const dataHourlyNotEmpty = !_.isNil(dataContext.dataBlackhole[canisterId])
    const dataDailyNotEmpty = !_.isNil(dataContext.dataBlackhole[canisterId])

    if (!dataHourlyNotEmpty && !dataDailyNotEmpty) {
        return <Result status="warning" title={`No data.`} subTitle={<>Please check if you set the Blackhole canister as one of your canister's controller.</>}/>
    }
...
export const MetricsCanisterSectionComponent = (props: Props) => {
    const canisterId = props.canisterId;
