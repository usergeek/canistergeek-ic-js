...
const LogMessagesDataProvider = (props) => {
    ...
    const getInfos = (0, react_1.useCallback)(async (canisterIds) => {
        try {
            ...
            const promises = lodash_1.default.map(canisterIds, (canisterId) => fetchCanisterInfo(canisterId, props.identity, props.host));
    ...
    const getLogMessages = (0, react_1.useCallback)(async (parameters) => {
        ...
        const promises = lodash_1.default.map(parameters.canisters, async (canisterContext) => {
            const canisterActor = CanistergeekService_1.CanistergeekService.createCanistergeekCanisterActor(canisterContext.canisterId, props.identity, props.host);
...
const fetchCanisterInfo = async (canisterId, identity, host) => {
    const canisterActor = CanistergeekService_1.CanistergeekService.createCanistergeekCanisterActor(canisterId, identity, host);
    let result = [];
    try {
