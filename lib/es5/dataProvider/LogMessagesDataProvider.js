...
const LogMessagesDataProvider = (props) => {
    ...
    const getInfos = (0, react_1.useCallback)(async (canisterIds) => {
        try {
            ...
            const promises = lodash_1.default.map(canisterIds, (canisterId) => DataProvider_1.CanistergeekAPIHelper.getCanisterLogMessagesInfo(canisterId, props.createActorFn, props.identity, props.host));
...
const getCanisterLogRecursive = async (canisterId, canisterActor, request) => {
    ...
    while (true) {
        const loggerResponse = await DataProvider_1.CanistergeekAPIHelper.getCanisterLogResponse(canisterActor, request); //getCandidOptional<CanisterLogResponse>(getCanisterLogResult);
        if (loggerResponse && (0, typescriptAddons_1.hasOwnProperty)(loggerResponse, "messages")) {
            const logMessages = loggerResponse.messages;
            messagesResult.data = [
                ...messagesResult.data,
