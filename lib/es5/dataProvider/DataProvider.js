...
exports.CanistergeekAPIHelper = exports.DataProvider = exports.useDataContext = void 0;
    ...
    const collectCanisterMetrics = (0, react_1.useCallback)(async (params) => {
        try {
            ...
            const promises = lodash_1.default.map(params.canisterIds, async (canisterId) => {
                ...
                return exports.CanistergeekAPIHelper.collectCanisterMetrics(canisterActor, params.collectMetricsType || "normal");
            });
            const allSettledResult = await Promise.allSettled(promises);
            lodash_1.default.each(allSettledResult, (promiseSettleResult, idx) => {
...
const fetchCanisterHourlyDataAndUpdateState = async (params, createActorFn, identity, host) => {
    ...
    const hourlyData = await exports.CanistergeekAPIHelper.getCanisterMetrics(canisterActor, parameters);
...
const fetchCanisterDailyDataAndUpdateState = async (params, createActorFn, identity, host) => {
    ...
    const dailyData = await exports.CanistergeekAPIHelper.getCanisterMetrics(canisterActor, parameters);
...
const fetchCanisterBlackholeDataAndUpdateState = async (params) => {
    const canisterActor = (0, CanistergeekService_1.createBlackholeCanisterActor)();
...
exports.CanistergeekAPIHelper = (() => {
    ...
    const getCanisterMetrics = async (canisterActor, parameters) => {
        try {
            ...
            const request = { version: true, status: [], metrics: [metricsRequest], logs: [] };
            ...
            */
            ...
            if (CanistergeekService_1.ICCanisterResponseUtil.isICCanisterResponseQueryError_NoMethod(canisterResponseQueryError)) {
    ...
    const collectCanisterMetrics = async (canisterActor, collectMetricsType) => {
        try {
            const request = { version: true, status: [], metrics: [], logs: [] };
            const getCanistergeekInformationResponse = await canisterActor.getCanistergeekInformation(request);
            const canisterSupportedVersion = getCanisterSupportedVersion(getCanistergeekInformationResponse);
            if (ifCanisterSupportsVersion(canisterSupportedVersion, SUPPORTED_VERSION)) {
            ...
            if (CanistergeekService_1.ICCanisterResponseUtil.isICCanisterResponseQueryError_NoMethod(canisterResponseQueryError)) {
                ...
                const canisterResponseUpdateError = CanistergeekService_1.ICCanisterResponseUtil.parseICCanisterResponseUpdateError(e);
                if (CanistergeekService_1.ICCanisterResponseUtil.isICCanisterResponseUpdateError_NoUpdateMethod(canisterResponseUpdateError)) {
                    tryLegacy = true;
                }
            }
            if (tryLegacy) {
    ...
    const getCanisterLogMessagesInfo = async (canisterId, createActorFn, identity, host) => {
        ...
        if (canisterActor) {
            const logRequest = { getMessagesInfo: null };
            let tryLegacy = false;
            try {
                //trying to get version and logs
                const request = { version: true, status: [], metrics: [], logs: [logRequest] };
                const getCanistergeekInformationResponse = await canisterActor.getCanistergeekInformation(request);
                const canisterSupportedVersion = getCanisterSupportedVersion(getCanistergeekInformationResponse);
                if (ifCanisterSupportsVersion(canisterSupportedVersion, SUPPORTED_VERSION)) {
                    const result = getCanistersLogMessagesInfoData(getCanistergeekInformationResponse.logs);
                    if (result == undefined) {
                        tryLegacy = true;
                        // noinspection ExceptionCaughtLocallyJS
                ...
                * */
                if (!tryLegacy) {
                    const canisterResponseQueryError = CanistergeekService_1.ICCanisterResponseUtil.parseICCanisterResponseQueryError(e);
                    tryLegacy = CanistergeekService_1.ICCanisterResponseUtil.isICCanisterResponseQueryError_NoMethod(canisterResponseQueryError);
                }
                if (tryLegacy) {
                    console.warn(`CanistergeekAPIHelper.getCanisterLogMessagesInfo: fallback to legacy method getCanisterLog()...`);
                    const result = await canisterActor.getCanisterLog([logRequest]);
    ...
    const getCanisterLogResponse = async (canisterActor, logRequest) => {
        let tryLegacy = false;
        try {
            //trying to get version and logs
            const request = { version: true, status: [], metrics: [], logs: [logRequest] };
            ...
            const canisterSupportedVersion = getCanisterSupportedVersion(getCanistergeekInformationResponse);
            if (ifCanisterSupportsVersion(canisterSupportedVersion, SUPPORTED_VERSION)) {
                const result = (0, CanistergeekService_1.getCandidOptional)(getCanistergeekInformationResponse.logs);
                if (result == undefined) {
                    tryLegacy = true;
                    // noinspection ExceptionCaughtLocallyJS
                    throw new Error(`try legacy`);
            ...
            */
            ...
            if (tryLegacy) {
                ...
                return (0, CanistergeekService_1.getCandidOptional)(getCanisterLogResult);
            }
            console.error(`CanistergeekAPIHelper.getCanisterLogResponse: caught error`, e);
            throw e;
        }
    };
    const getCanistersLogMessagesInfoData = (logs) => {
        const loggerResponse = (0, CanistergeekService_1.getCandidOptional)(logs);
        if (loggerResponse) {
            if ((0, typescriptAddons_1.hasOwnProperty)(loggerResponse, "messagesInfo")) {
                const info = loggerResponse.messagesInfo;
                const firstTimeNanos = (0, CanistergeekService_1.getCandidOptional)(info.firstTimeNanos);
                const lastTimeNanos = (0, CanistergeekService_1.getCandidOptional)(info.lastTimeNanos);
                return {
                    count: info.count,
