...
const DataProvider = (props) => {
    ...
    const getCanisterMetrics = (0, react_1.useCallback)(async (params) => {
        const get = async () => {
            updateContextStatus(lodash_1.default.mapValues(lodash_1.default.mapKeys(params, v => v.canisterId), () => ({ inProgress: true })));
            try {
    ...
    const collectCanisterMetrics = (0, react_1.useCallback)(async (params) => {
        try {
            ...
            const promises = lodash_1.default.map(params.canisterIds, async (canisterId) => {
                const canisterActor = CanistergeekService_1.CanistergeekService.createCanisterActor(canisterId, props.identity, props.host);
...
const fetchCanisterHourlyDataAndUpdateState = async (params, identity, host) => {
    const canisterActor = CanistergeekService_1.CanistergeekService.createCanisterActor(params.canisterId, identity, host);
